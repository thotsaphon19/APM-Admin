...
export default function Leave() {
  ...
  const { notifyLeaveRequest, notifyLeaveApproved, notifyLeaveRejected } = useNotify()
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      await approveLeave(id, profile?.id)
      // แจ้งแอดมินคนที่ขอลาโดยตรง
      notifyLeaveResult(
        leave?.employeeId,
        getUserName(leave?.employeeId),
        true,
        ''
      )
    }
  ...
  const handleReject = async (id, reason='') => {
    ...
    try {
      const leave = leaves.find(l => l.id === id)
      await rejectLeave(id, profile?.id)
      // แจ้งแอดมินคนที่ขอลาโดยตรง
      notifyLeaveResult(
        leave?.employeeId,
        getUserName(leave?.employeeId),
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button className={`btn btn-ghost btn-sm ${viewMode === 'history' ? 'bg-indigo-100' : ''}`}
      ...
      {!isAdmin && stats.pending > 0 && (
        <div className="rounded-xl bg-orange-500/10 border border-orange-500/30 p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <span className="text-sm text-gray-500 ml-1">รออนุมัติ — กรุณาตรวจสอบ</span>
          </div>
    ...
    <div className={`card p-0 overflow-hidden border ${st.bg} transition-all`}>
      ...
      <div className="flex flex-wrap items-center gap-4 p-4">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
              ...
              <span className="font-semibold text-gray-500">{days} วัน</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 hidden sm:block">
          <div className="text-xs text-gray-500 font-semibold mb-0.5">{leaveTypeLabel(l.leaveType)}</div>
      ...
      {/* Expanded detail */}
      {expanded && (
        <div className="border-t border-indigo-100/60 bg-white px-5 py-4 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
  ...
  return (
    <Modal open onClose={onClose} title="ยื่นขอลา">
      ...
      <div className={`rounded-xl border p-3 mb-4 flex items-center gap-3 text-sm ${
        ...
        <span className="text-gray-500">จำนวนวันลา:</span>

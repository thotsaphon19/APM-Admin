...
function ScreenVerify({ email, onDone }) {
  ...
  async function verify() {
    ...
    setBusy(true); setErr('');
    try {
      await api.verifyEmailOTP({ email, otp, type: 'verify' });
      onDone();
    } catch(ex){ setErr(ex.message); }
    finally{ setBusy(false); }
  }
  ...
  return (
    <div className="lp-form">
      ...
      <p className="lp-sub">ระบบส่ง OTP 6 หลักไปที่<br/><strong>{email}</strong></p>
...
export default function LoginPage() {
  const [screen, setScreen] = useState('login'); // login | verify | forgot
  const [verifyEmail, setVerifyEmail] = useState('');

  // After login if unverified → show verify
  // (LoginPage is shown only when !user, AuthContext handles routing)
  ...
  return (
    <div className="lp-page">
      ...
      <div className="lp-card">
        ...
        {screen === 'login'  && <ScreenLogin  onForgot={() => setScreen('forgot')}/>}
        {screen === 'verify' && <ScreenVerify email={verifyEmail} onDone={() => setScreen('login')}/>}
        {screen === 'forgot' && <ScreenForgot onBack={() => setScreen('login')}/>}
      </div>

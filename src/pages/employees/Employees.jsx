          ...
          return (
            <div key={r.role}
              className={`card cursor-pointer border transition-all hover:shadow-lg ${
                filter === r.role ? `${c.border} ${c.bg}` : 'hover:border-gray-700'
              }`}
        ...
        : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(emp => {
              ...
              return (
                <div key={emp.id} className={`card border-t-2 hover:border-gray-700 transition-all ${c.border}`}>
                  {/* Top */}
            ...
            return (
              <label key={role} onClick={() => setCurrent(p => ({...p, role}))}
                className={`flex items-center gap-3 p-3.5 rounded-xl cursor-pointer border transition-all ${
                  selected ? `${c.bg} ${c.border}` : 'bg-gray-800/40 border-gray-700/50 hover:border-gray-600'
                }`}>

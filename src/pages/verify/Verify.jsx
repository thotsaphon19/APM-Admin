    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className={`card text-center ${stats.errors > 0 ? 'border-red-500/30 bg-red-500/5' : ''}`}>
          ...
          <div className={`text-2xl font-black ${stats.errors > 0 ? 'text-red-400' : 'text-gray-500'}`}>{stats.errors}</div>
        </div>
        <div className={`card text-center ${stats.warnings > 0 ? 'border-orange-500/30 bg-orange-500/5' : ''}`}>
          ...
          <div className={`text-2xl font-black ${stats.warnings > 0 ? 'text-orange-400' : 'text-gray-500'}`}>{stats.warnings}</div>
        </div>
              ...
              <div key={u.id} className={`rounded-xl border p-3 ${
                ...
                <div className="grid grid-cols-3 gap-1 text-center">
                  <div>
                    ...
                    <div className={`text-sm font-black ${u.errors > 0 ? 'text-red-400' : 'text-gray-500'}`}>{u.errors}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-gray-500">น่าสงสัย</div>
                    <div className={`text-sm font-black ${u.warnings > 0 ? 'text-orange-400' : 'text-gray-500'}`}>{u.warnings}</div>
                  </div>
                  <div>
                ...
                <div className="mt-2">
                  ...
                  <div className="h-1 bg-indigo-50 rounded-full overflow-hidden flex">
                    {(u.manual + u.ai) > 0 && (
                      <>
      ...
      <div className="card space-y-3">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button className="btn btn-primary btn-sm"
            ...
            style={{ background:'linear-gradient(135deg,#6366f1,#7c3aed)', border:'none', borderRadius:9, padding:'7px 18px', cursor:'pointer', fontSize:13, fontWeight:800, color:'#fff', fontFamily:'inherit', display:'flex', alignItems:'center', gap:6 }}>
          ...
          {displayed.map(c => {
            ...
            return (
              <div key={c.id} className={`card p-0 overflow-hidden border transition-all ${statusBg(c.issues)}`}>
                {/* Main row */}
                <div className="flex flex-wrap items-center gap-3 p-4 cursor-pointer hover:bg-indigo-50"
                ...
                {/* Expanded detail */}
                {exp && (
                  <div className="border-t border-indigo-100/60 bg-white p-4 space-y-4">

                    {/* Issue list */}
                    {c.issues.length > 0 && (
                      <div className="space-y-2">
                        <div className="text-xs font-bold text-gray-500 uppercase tracking-wider">ผลการตรวจสอบ</div>
                    ...
                    {/* Detail table */}
                    <div>
                      <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">รายละเอียดข้อมูล</div>
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        <VerifyBox
                          label="ตอบมือ (บ้าน)"
                    ...
                    <div className="rounded-lg bg-indigo-50 border border-indigo-200/50 p-3 text-xs text-gray-500">

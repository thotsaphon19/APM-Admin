...
function updateProfile(data) {
  const { email, firstName, lastName, phone, lineId, position, bio } = data;
  const sh    = ss.getSheetByName(SHEETS.USERS);
  const users = getSheetData(SHEETS.USERS);
  ...
  // Update basic fields
  const fieldMap = { firstName, lastName, phone, lineId, position, bio };
  Object.entries(fieldMap).forEach(([f, v]) => {
    if (v !== undefined) {
      const col = hdrs.indexOf(f) + 1;
      if (col > 0) sh.getRange(rowNum, col).setValue(v);
    }
  });
  ...
  const dnCol = hdrs.indexOf('displayName') + 1;
  if (dnCol > 0) {
    sh.getRange(rowNum, dnCol).setValue(`${firstName||users[idx].firstName||''} ${lastName||users[idx].lastName||''}`.trim());
  }
